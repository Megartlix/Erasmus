...
# Veritabanına işçi ekleme fonksiyonu
def add_worker_to_db(worker_id, username, password):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO kullanicilar (id, kullanici_adi, sifre, rol) VALUES (?, ?, ?, ?)", 
                   (worker_id, username, password, 'işçi'))
    conn.commit()
    conn.close()

# İşçi Ekleme Ekranı
class WorkerAddScreen(QMainWindow):
    def __init__(self):
        super().__init__()

        self.setWindowTitle('İşçi Ekle')
        ...
        # Başlık
        self.header_label = QLabel('Yeni İşçi Ekle')
        ...
        self.worker_id_input.setPlaceholderText("İşçi ID")
    ...
    def add_worker(self):
        ...
        # Veritabanına işçiyi ekle
        try:
            add_worker_to_db(worker_id, username, password)
            QMessageBox.information(self, 'Başarılı', 'İşçi başarıyla eklendi!')
            self.clear_fields()
        except Exception as e:
            QMessageBox.warning(self, 'Hata', f'İşçi eklenirken bir hata oluştu: {e}')
...
if __name__ == '__main__':
    ...
    # İşçi Ekleme ekranını göster
    window = WorkerAddScreen()
    window.show()
    sys.exit(app.exec_())
